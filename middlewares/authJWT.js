...
export const validarJWT = (req, res, next) => {
    ...
    try {
        ...
        if (req.params.id && req.user.uid !== req.params.id) {
            ...
            return res.status(402).json({ msg: "El usuario no tiene permisos para realizar esta acción sobre este ID" });
        }
        ...
        res.status(403).json({ msg: "ese token no vale caremonda" })
    }
}
...
export const validarAdminJWT = (req, res, next) => {
    ...
    try {
        ...
        if (role !== "admin") {
            return res.status(402).json({ msg: "Permisos insuficientes" })
        }
        next();
    } catch (error) {
        console.log("Error en validarAdminJWT:", error);
